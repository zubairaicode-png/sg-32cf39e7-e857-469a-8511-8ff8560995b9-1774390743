...
export default function NewSalesReturnPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      toast({
        ...
        description: "Credit note has been issued to customer.",
      });
  ...
  return (
    <>
      <SEO title="Create Sales Return - Maka ERP" />
      <DashboardLayout>
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">Create Sales Return</h1>
              <p className="text-muted-foreground mt-1">Issue credit note for returned goods</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mt-6 flex justify-end">
                  <div className="w-64 space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-destructive">-SAR {subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Total VAT:</span>
                      <span className="font-semibold text-destructive">-SAR {totalVat.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-lg font-bold pt-2 border-t">
                      <span>Grand Total:</span>
                      <span className="text-destructive">-SAR {grandTotal.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="flex items-center gap-4">
              <Button type="submit" disabled={isLoading} className="gap-2">
                ...
                {isLoading ? "Processing..." : "Create Sales Return"}
              </Button>
              <Link href="/sales/returns">
